Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/42b352fa63eb15746a369baa6d2b1ba6"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyPathing(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#endif
				float4 PathColor[4] = {
					float4(0.4858491,0.5002916,1,1), float4(1,1,1,1), float4(1,1,1,1), float4(1,1,1,1)
				};
				...
				for (int i = 0; i < 4; i++)
				{
					...
					float timeOffset = 0;
					#endif
					float currentTime = frac((float4(-999,-999,-999,-999)[i] == -999.0f ? _Time.y * float4(0.3,1,1,0.1)[i] : float4(-999,-999,-999,-999)[i]) + float4(0,0,0,0)[i] + timeOffset);
					...
					#ifdef POI_AUDIOLINK
					half pathWidth = float4(0.93,0.03,0.03,0.03)[i] * 0.5 + (0.0 ? alWidthOffsets[i] : 0);
					#else
					half pathWidth = float4(0.93,0.03,0.03,0.03)[i] * 0.5;
					#endif
					...
					half dashAlpha = 0;
					float totalDashLength = float4(0.93,0.03,0.03,0.03)[i] + float4(0.1,0.1,0.1,0.1)[i];
					if (totalDashLength > 1e-6)
					{
						float relativeDashLength = float4(0.93,0.03,0.03,0.03)[i] / totalDashLength;
